...
export default function Widgets({ newsResults, randomUsersResults }) {
  ...
  useEffect(
    () =>
      onSnapshot(
        query(collection(db, "contact",session?.user?.uid||"0123","follow"), orderBy("timestamp", "desc")),
        (snapshot) => {
    ...
    <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5">
      ...
      <div className="text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 w-[90%] xl:w-[75%]">
        ...
        <button
          ...
          className="text-xl text-blue-400 pl-4 pb-3 hover:text-blue-400"
        >
          Xem thêm
        </button>
      </div>
      <div className="sticky top-16 text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[75%]">
        ...
        <AnimatePresence>
        {listFollow?.slice(0, randomUserNum).map((follower.data()) => (
           <motion.div
          ...
          >
            ...
            <div className="truncate ml-4 leading-5">
              ...
              <h5 className="text-[13px] text-gray-500 truncate">
                {follower.data().username + " " +follower.data().username}
              </h5>
            </div>
