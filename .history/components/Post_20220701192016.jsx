...
export default function Post({ post, id }) {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      collection(db, "contact", session?.user?.id, "follow"),
  ...
  useEffect(() => {
    setHasFollowed(
      follower.findIndex((follow) => follow.id === session?.user?.uid) !== -1
    );
  }, [follower]);
        ...
        <Dropdown placement="bottom-left">
          ...
          <Dropdown.Menu color="secondary" aria-label="Avatar Actions">
            <Dropdown.Item key="profile" css={{ height: "$18" }}>{
              hasFollowed?( <Text b color="inherit" css={{ d: "flex" }} onClick={follow}>
              Theo dõi
            </Text>
            <Text b color="inherit" css={{ d: "flex" }}>
              @{post?.data().username}
            </Text>):( <Text b color="inherit" css={{ d: "flex" }} onClick={follow}>
              Theo dõi
            </Text>
            <Text b color="inherit" css={{ d: "flex" }}>
              @{post?.data().username}
