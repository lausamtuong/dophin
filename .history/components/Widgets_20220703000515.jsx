  ...
  return (
    <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5">
      ...
      <div className="  text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[75%]">
        <h4 className="font-bold text-xl px-4">Đang hoạt động</h4>
            {session&&
        <AnimatePresence>
            ...
            <button 
             className="ml-auto bg-black text-white rounded-full text-sm px-3.5 py-1.5 font-bold"
              onClick={async()=>{
                ...
                await setDoc(doc(db, "chat",Number(currentUser.uid)+Number(Onlline.data().uid, "likes_noti", session?.user?.uid), session.user);
                setHidden(!hidden)
      ...
      <Chatbox hidden={hidden} currentUser={currentUser} other={other}/>
    </div>
  );
}
